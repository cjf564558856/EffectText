...
@property (nonatomic, strong) UIImage *assistOneImage;
@property (nonatomic, strong) UIImage *assisttwoImage;

@property (nonatomic, strong) NSMutableArray *imageArray;
@property (nonatomic, assign) NSUInteger imageIndex;
@property (nonatomic, assign) BOOL isTextCode;
@property (nonatomic, copy) NSString * shName;
...
- (void)commonInit {
    ...
    NSString *maskPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"mask2.png"];
...
//    self.viewImage = [FaceUtils faceDetectWithImage:[UIImage imageWithContentsOfFile:imagePath]];
    ...
    self.assistOneImage = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"filter.png"]];
    self.assisttwoImage = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"916.png"]];
...
- (void)createImageDataWithImageArray:(NSArray *)imageArray {
...
- (void)updateTextureWithImageIndex:(NSInteger)imageIndex{

    CGImageRef cgImageRef = [self.imageArray[imageIndex] CGImage];
    GLuint width = (GLuint)CGImageGetWidth(cgImageRef);
    GLuint height = (GLuint)CGImageGetHeight(cgImageRef);

    glBindTexture(GL_TEXTURE_2D, assistOneTexture);
//    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, frame[imageIndex]);
    
//    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, frame[imageIndex]);

    
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, frame[imageIndex]);

    glBindTexture(GL_TEXTURE_2D, 0);
    
    self.texture5ID = assistOneTexture;
}
...
- (void)setupFilterBar {
    ...
    NSArray *dataSource = @[@"无", @"缩放", @"灵魂出窍", @"抖动", @"闪白", @"毛刺", @"幻觉", @"放大", @"旋转", @"下降", @"边缘", @"荧光", @"交错", @"丁达尔", @"彩色线条", @"相机焦点", @"彩虹", @"高斯模糊", @"定向模糊", @"径向模糊", @"锐化", @"边角定位", @"贝塞尔变形",@"频闪",@"黑白三屏",@"九屏跑马灯",@"彩虹光",@"萤火",@"模糊星光2",@"撒星星",@"撒星星2",@"闪亮登场",@"闪亮登场2",@"闪耀星光",@"少女星闪",@"圣诞星光",@"糖果纸",@"温柔细闪",@"细闪",@"细闪2",@"细闪3",@"闪闪",@"夸夸弹幕",@"哈哈弹幕",@"裂开了",@"飞速计算",@"高光瞬间",@"胡言乱语",@"满屏问号",@"凄凉",@"我酸了",@"乌鸦飞过",@"不对劲",@"变形了",@"闪烁"];
...
- (void)timeAction {
    ...
    if(self.currentIndex == 10 || self.currentIndex == 11 || self.currentIndex == 14 || self.currentIndex == 23 || self.currentIndex == 41) {
        //第一次渲染将模型绘制到FBO上
...
//                    glUniformMatrix4fv(transformSlot, 1, GL_FALSE, (GLfloat *)&T);
    ...
    if(self.currentIndex == 10 || self.currentIndex == 11 || self.currentIndex == 14 || self.currentIndex == 23 || self.currentIndex == 41) {
        ...
        if (self.currentIndex == 41) {
            GLuint textureSlot2 = glGetUniformLocation(self.program2, "maskTexture");
            glActiveTexture(GL_TEXTURE2);
            glBindTexture(GL_TEXTURE_2D, self.texture3ID);
            glUniform1i(textureSlot2, 2);
            
            GLuint textureSlot3 = glGetUniformLocation(self.program2, "_MainTex");
            glActiveTexture(GL_TEXTURE3);
            glBindTexture(GL_TEXTURE_2D, self.texture5ID);
            glUniform1i(textureSlot3, 3);
...
- (void)filterBar:(FilterBar *)filterBar didScrollToIndex:(NSUInteger)index {
    ...
    if (index == 0) {
        ...
        [self setupStroboFlashShaderProgram];
    }
    else if(index == 24) {
        self.currentIndex = 24;
        [self setupBlackWhiteThreeGeShaderProgram];
    }
    else if(index == 25) {
        self.currentIndex = 25;
        [self setupNineScreenPMDShaderProgram];
    }
    else if(index == 26) {
        self.currentIndex = 26;
        [self setupCHGShaderProgram];
    }
    else if(index == 27) {
        self.currentIndex = 27;
        [self setupYHShaderProgram];
    }
    else if(index == 28) {
        self.currentIndex = 28;
        [self setupMHXG2haderProgram];
    }
    else if(index == 29) {
        self.currentIndex = 29;
        [self setupSXXhaderProgram];
    }
    else if(index == 30) {
        self.currentIndex = 30;
        [self setupSXX2haderProgram];
    }
    else if(index == 31) {
        self.currentIndex = 31;
        [self setupSLDChaderProgram];
    }
    else if(index == 32) {
        self.currentIndex = 32;
        [self setupSLDC2haderProgram];
    }
    else if(index == 33) {
        self.currentIndex = 33;
        [self setupSYXGhaderProgram];
    }
    else if(index == 34) {
        self.currentIndex = 34;
        [self setupSNXSShaderProgram];
    }
    else if(index == 35) {
        self.currentIndex = 35;
        [self setupSDXGShaderProgram];
    }
    else if(index == 36) {
        self.currentIndex = 36;
        [self setupTGZShaderProgram];
    }
    else if(index == 37) {
        self.currentIndex = 37;
        [self setupWRXSShaderProgram];
    }
    else if(index == 38) {
        self.currentIndex = 38;
        [self setupXSShaderProgram];
    }
    else if(index == 39) {
        self.currentIndex = 39;
        [self setupXS2ShaderProgram];
    }
    else if(index == 40) {
        self.currentIndex = 40;
        [self setupXS3ShaderProgram];
    }
    else if(index == 41) {
        self.currentIndex = 41;
        [self setupSSShaderProgram];
    }
    else if(index == 42) {
        self.currentIndex = 42;
        [self setupKKDMhaderProgram];
    }
    else if(index == 43) {
        self.currentIndex = 43;
        [self setupHHDMhaderProgram];
    }
    else if(index == 44) {
        self.currentIndex = 44;
        [self setupLKLShaderProgram];
    }
    else if(index == 45) {
        self.currentIndex = 45;
        [self setupFSJSShaderProgram];
    }
    else if(index == 46) {
        self.currentIndex = 46;
        [self setupGGSJShaderProgram];
    }
    else if(index == 47) {
        self.currentIndex = 47;
        [self setupHYLYShaderProgram];
    }
    else if(index == 48) {
        self.currentIndex = 48;
        [self setupMPWHShaderProgram];
    }
    else if(index == 49) {
        self.currentIndex = 49;
        [self setupQLShaderProgram];
    }
    else if(index == 50) {
        self.currentIndex = 50;
        [self setupLMShaderProgram];
    }
    else if(index == 51) {
        self.currentIndex = 51;
        [self setupWYFGShaderProgram];
    }
    else if(index == 52) {
        self.currentIndex = 52;
        [self setupBDJShaderProgram];
    }
    else if(index == 53) {
        self.currentIndex = 53;
        [self setupBXLShaderProgram];
    }
    else if(index == 54) {
        self.currentIndex = 54;
        [self setupShanShuoShaderProgram];
    }
    // 重新开始计算时间
    [self startFilerAnimation];
}
...
- (void)setupColorEdgeShaderProgram {
    [self setupRDShaderProgramWithName:@"ColorEdge"];
}
...
// 贝塞尔着色器程序
- (void)setupBingdongShaderProgram {
    [self setupRDShaderProgramWithName:@"bingdong"];
}

//频闪
- (void)setupStroboFlashShaderProgram {
    [self setupRDShaderProgramWithName:@"StroboFlash"];
}

//黑白三屏
- (void)setupBlackWhiteThreeGeShaderProgram {
    [self setupRDShaderProgramWithName:@"BlackWhiteThreeGe"];
}

//九屏跑马灯
- (void)setupNineScreenPMDShaderProgram {
    [self setupRDShaderProgramWithName:@"NineScreenPMD"];
}

//彩虹光
- (void)setupCHGShaderProgram {
    self.isTextCode = YES;

    [self setupRDShaderProgramWithName:@"彩虹光"];
}

//萤火
- (void)setupYHShaderProgram {
    self.isTextCode = YES;

    [self setupRDShaderProgramWithName:@"萤火"];
}

//模糊星光2
- (void)setupMHXG2haderProgram {
    self.isTextCode = YES;

    [self setupRDShaderProgramWithName:@"模糊星光2"];
}

//撒星星
- (void)setupSXXhaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"撒星星"];
}

//撒星星2
- (void)setupSXX2haderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"撒星星2"];
}

//闪亮登场
- (void)setupSLDChaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"闪亮登场"];
}

//闪亮登场2
- (void)setupSLDC2haderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"闪亮登场2"];
}

//闪耀星光
- (void)setupSYXGhaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"闪耀星光"];
}

//少女星闪
- (void)setupSNXSShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"少女星闪"];
}

//圣诞星光
- (void)setupSDXGShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"圣诞星光"];
}

//糖果纸
- (void)setupTGZShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"糖果纸"];
}

//温柔细闪
- (void)setupWRXSShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"温柔细闪"];
}

//细闪
- (void)setupXSShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"细闪"];
}

//细闪2
- (void)setupXS2ShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"细闪2"];
}

//细闪3
- (void)setupXS3ShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"细闪3"];
}

//闪闪
- (void)setupSSShaderProgram {
    self.isTextCode = YES;
    [self setupTwoShadeProgramWithFirstFilter:@"pass0" secondFilter:@"闪闪"];
}

//夸夸弹幕
- (void)setupKKDMhaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"夸夸弹幕"];
}

//哈哈弹幕
- (void)setupHHDMhaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"哈哈弹幕"];
}

//裂开了
- (void)setupLKLShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"裂开了"];
}

//飞速计算
- (void)setupFSJSShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"飞速计算"];
}

//高光瞬间
- (void)setupGGSJShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"高光瞬间"];
}

//胡言乱语
- (void)setupHYLYShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"胡言乱语"];
}

//满屏问号
- (void)setupMPWHShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"满屏问号"];
}

//凄凉
- (void)setupQLShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"凄凉"];
}

//我酸了
- (void)setupLMShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"我酸了"];
}

//乌鸦飞过
- (void)setupWYFGShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"乌鸦飞过"];
}

//不对劲
- (void)setupBDJShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"不对劲"];
}

//变形了
- (void)setupBXLShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"变形了"];
}

//闪烁
- (void)setupShanShuoShaderProgram {
    self.isTextCode = YES;
    [self setupRDShaderProgramWithName:@"ShanShuo"];
}

//// 聚光灯着色器程序
//- (void)setupSpotlightShaderProgram {
//    [self setupShaderProgramWithName:@"Spotlight"];
//}
//
//// 马赛克着色器程序
...
- (void)setupTwoShadeProgramWithFirstFilter:(NSString *)firstFilter secondFilter:(NSString *)secondFilter{
    GLuint program = [self programWithShaderName:firstFilter];
    GLuint program2 = [self programWithShaderName:secondFilter];

    if (self.currentIndex == 41) {
        
        filterTexture = [self createTextureWithImage:self.assistOneImage];

        glUseProgram(program);

        GLuint positionSlot = glGetAttribLocation(program2, "position");
        GLuint textureCoordsSlot = glGetAttribLocation(program,"inputTextureCoordinate");
        GLuint textureSlot = glGetUniformLocation(program2, "inputImageTexture");
        GLuint texture2Slot = glGetUniformLocation(program2, "inputImageTexture2");
        ...
        for (int i = 0; i < 15; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"ss%d",i]]];
        }
        
        assistOneTexture = [self createTextureWithImage:self.imageArray[0]];
        self.texture5ID = assistOneTexture;
    }
...
- (void)setupRDShaderProgramWithName:(NSString *)name {
    ...
    if ([self.shName isEqualToString:@"彩虹光"]) {
        for (int i = 0; i <  60; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"a_%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"萤火"]) {
        for (int i = 0; i <  47; i++) {
            if (i < 10) {
                [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"yg_000%d",i]]];
            }else{
                [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"yg_00%d",i]]];
            }
        }
    }else if ([self.shName isEqualToString:@"模糊星光2"]) {
        for (int i = 0; i <  59; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"a%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"撒星星"]) {
        for (int i = 0; i <  78; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"1a%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"撒星星2"]) {
        for (int i = 0; i <  63; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"b%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"闪亮登场"]) {
        for (int i = 0; i < 47; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"sldc%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"闪亮登场2"]) {
        for (int i = 0; i < 49; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"sldcTwo%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"闪耀星光"]) {
        for (int i = 0; i < 47; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"syxg%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"少女星闪"]) {
        for (int i = 0; i < 47; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"snxs%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"圣诞星光"]) {
        for (int i = 0; i < 44; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"sdxg%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"糖果纸"]) {
        for (int i = 0; i < 55; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"tgz%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"温柔细闪"]) {
        for (int i = 0; i < 79; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"wrxs%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"细闪"]) {
        for (int i = 0; i < 47; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"xs%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"细闪2"]) {
        for (int i = 0; i < 47; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"xsTwo%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"细闪3"]) {
        for (int i = 0; i < 44; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"xsThree%d",i]]];
        }
    }else if (self.currentIndex == 41) {
        for (int i = 0; i < 15; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"ss%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"夸夸弹幕"]) {
        for (int i = 0; i < 63; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"kkdm%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"哈哈弹幕"]) {
        for (int i = 0; i < 78; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"hhdm%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"飞速计算"]) {
        for (int i = 0; i < 79; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"fsjs_%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"高光瞬间"]) {
        for (int i = 0; i < 52; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"gaoguang_0%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"胡言乱语"]) {
        for (int i = 0; i < 48; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"hyly_%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"满屏问号"]) {
        for (int i = 0; i < 34; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"mpwh_%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"凄凉"]) {
        for (int i = 0; i < 31; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"qil_%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"我酸了"]) {
        for (int i = 0; i < 51; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"lm%d",i]]];
        }
    }else if ([self.shName isEqualToString:@"乌鸦飞过"]) {
        for (int i = 0; i < 74; i++) {
            [self.imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"wy%d",i]]];
        }
    }
    ...
    if (_isTextCode) {
        ...
        if (self.imageArray.count > 0) {
            [self createImageDataWithImageArray:self.imageArray];
        ...
        //细闪
        if (self.currentIndex == 40) {
            GLuint bgTexture = [self createTextureWithImage:[UIImage imageNamed:@"xsThreeBg"]];
            self.texture6ID = bgTexture;
...
//            glUniformMatrix4fv(transformSlot, 1, GL_FALSE, rotateMatrix.m);
    ...
    GLuint textureSlot = glGetUniformLocation(program, "inputImageTexture");
    GLuint textureSlot2 = glGetUniformLocation(program, "inputImageTexture2");
