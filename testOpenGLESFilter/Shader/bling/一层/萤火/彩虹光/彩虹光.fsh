...
const int blendMode = 1;

const float alphaFactor = 2.0;
const float sizeIntensity = 1.0;
const float horzIntensity = 0.0;
const float vertIntensity = 0.2;
...
#define textureCoordinate textureCoordinate
#define inputImageTexture2 _MainTex

void main(void)
{
    int textureIndex = 266;
    float xPosition = 0.043868;
    float vertBrightness = 1.000000;
    float vertOpacity = 0.985306;
    float horizOpacity = .014694;
    float speed = 7.0;
    vec2 uPosStar = vec2(0.55,0.5445);
    vec2 uPosEnd = vec2(0.55,0.5445);
    
    float il=float(textureIndex)/24.0;
    float tl=mod(il,speed)/speed;
    tl*=3.1415926*2.0;
    tl=-cos(tl)*0.5+0.5;
    vec2 pos =mix(uPosStar,uPosEnd,vec2(tl));
    pos -= 0.65;
    
    float multX = xPosition;
    
    vec4 color = texture2D(inputImageTexture, textureCoordinate);
    vec2 uvOffset = ((color.rg + multX) * -0.12) - pos;
    //uvOffset.x = -uvOffset.x;
    vec2 uvDist = vec2(textureCoordinate.x,1.0-textureCoordinate.y) + uvOffset;
    uvDist.y = 1.0- uvDist.y;
    //vec2 uvDist = textureCoordinate + ((color.rg + multX) * -0.12) - pos;
    
    uvDist = textureCoordinate+(uvDist - textureCoordinate) * 0.5;
    uvDist.y = 1.0-uvDist.y;
    vec2 uv1 = (uvDist-.5+ vec2(-horzIntensity, vertIntensity) * 1.)/sizeIntensity+.5;
    vec4 spotColor = texture2D(inputImageTexture2, uv1 );
    if (uv1.x < 0. || uv1.x > 1. || uv1.y < 0. ||uv1.y > 1.) spotColor = vec4(0.);

    vec4 fgColor = spotColor;//texture2D(_MainTex, textureCoordinate);
    ...
    vec3 resultColor = blendFunc(bgColor.rgb, clamp(fgColor.rgb * (1.0 / (fgColor.a + .0001)), 0.0, 1.0), alphaFactor,newBlendMode);
    gl_FragColor = vec4(bgColor.rgb * (1.0 - fgColor.a) + resultColor.rgb * fgColor.a, 1.0);
}
