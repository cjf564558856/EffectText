...
const vec2 fullBlendTexSize = vec2(300.0,300.0);
...
vec4 blendColor_rotate(sampler2D sucai, vec4 baseColor,vec2 videoSize,vec2 sucaiSize,vec2 anchorImageCoord,float sucaiScale,float radianAngle)
{
    ...
    vec2 rotateUV = rotate(textureCoordinate * videoSize,anchorImageCoord,radianAngle)/videoSize;
    ...
    lowp vec4 fgColor = baseColor;
     ...
     if(sucaiUV.x >= 0.0 && sucaiUV.x <= 1.0 && sucaiUV.y >= 0.0 && sucaiUV.y <= 1.0 ) {
        sucaiUV.y = 1.0 - sucaiUV.y;    //amazing-enginesucaishangxiaflip
...
void main(void)
{
    ...
    // float fullBlendLongSide = max(baseTexureSize.x,baseTexureSize.y);
    // float scale = 1.0;
    // if(distance(fullBlendLongSide,baseTexureSize.x) < 1.0) {
    //     scale = fullBlendLongSide / fullBlendTexSize.x;
    // } else {
    //     scale = fullBlendLongSide / fullBlendTexSize.y;
    // }
    float scale = 1.0;
